...
class TestCaseCourseViews(APITestCase, CacheTestCase):
    ...
    def test_delete_course_schedule_by_id(self):
        ...
        self.assertFalse(CourseSchedule.objects.filter(pk=course_schedule.pk).exists())
    ...
    def test_filter_id_cannot_access_other_users_data(self):
        ...
        # WHEN
        response = self.client.get(reverse('planner_courseschedules_list') + f'?id={course_schedule.pk}')
    ...
    def test_create_schedule_from_template_expands_and_stores(self):
        # GIVEN
        user = userhelper.given_a_user_exists_and_is_authenticated(self.client)
        course_group = coursegrouphelper.given_course_group_exists(user)
        course = coursehelper.given_course_exists(course_group)
        ...
        data = {
            ...
            'cycle_slots': [
                ...
                {'indices': [2], 'start_time': '10:00:00', 'end_time': '10:50:00'},
            ],
        }
        response = self.client.post(
            reverse('planner_coursegroups_courses_courseschedules_list',
        ...
        self.assertTrue(schedule.is_cycle)

    def test_create_schedule_without_template_stores_null(self):
        # GIVEN
        user = userhelper.given_a_user_exists_and_is_authenticated(self.client)
        course_group = coursegrouphelper.given_course_group_exists(user)
        course = coursehelper.given_course_exists(course_group)
    ...
    def test_switch_template_clears_opposite_rotation_type(self):
        ...
        course_group = coursegrouphelper.given_course_group_exists(user)
        course = coursehelper.given_course_exists(course_group)
        schedule = courseschedulehelper.given_week_based_schedule_exists(course)

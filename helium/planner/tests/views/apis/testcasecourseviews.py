...
class TestCaseCourseViews(APITestCase):
    ...
    def test_updated_at_filter(self):
        ...
        Course.objects.filter(pk=course1.pk).update(updated_at=old_time)
        Course.objects.filter(pk=course2.pk).update(updated_at=recent_time)
        Course.objects.filter(pk=course3.pk).update(updated_at=recent_time)

        # WHEN
        filter_time = '2024-01-01T00:00:00'
        response = self.client.get(
            reverse('planner_courses_list') + f'?updated_at__gte={filter_time}')

        # THEN
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 2)
        returned_ids = {item['id'] for item in response.data}
        self.assertIn(course2.pk, returned_ids)
        self.assertIn(course3.pk, returned_ids)
        self.assertNotIn(course1.pk, returned_ids)
    ...
    def test_shown_on_calendar_filter(self):
        ...
        # WHEN
        response = self.client.get(reverse('planner_courses_list') + '?shown_on_calendar=true')

        # THEN
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        returned_ids = {item['id'] for item in response.data}
        self.assertIn(visible_course.pk, returned_ids)
        self.assertNotIn(hidden_course.pk, returned_ids)
    ...
    def test_filter_id_cannot_access_other_users_data(self):
        ...
        # WHEN
        response = self.client.get(reverse('planner_courses_list') + f'?id={course.pk}')

        # THEN
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(len(response.data), 0)

    def test_patch_course_exceptions_outside_date_range_returns_400(self):
        # GIVEN
        user = userhelper.given_a_user_exists_and_is_authenticated(self.client)
        course_group = coursegrouphelper.given_course_group_exists(user)
        course = coursehelper.given_course_exists(course_group)

...
class UserMaterialsApiListView(HeliumAPIView, ListModelMixin):
    ...
    def get_queryset(self):
        if hasattr(self.request, 'user') and not getattr(self, "swagger_fake_view", False):
            ...
            materials = Material.objects.for_user(user.pk).select_related('material_group').prefetch_related('courses', 'note_links__note')
...
class MaterialGroupMaterialsApiListView(HeliumAPIView, CreateModelMixin, ListModelMixin):
    ...
    def get_queryset(self):
        if hasattr(self.request, 'user') and not getattr(self, "swagger_fake_view", False):
            user = self.request.user
            return Material.objects.for_user(user.pk).for_material_group(self.kwargs['material_group']).select_related('material_group').prefetch_related('courses', 'note_links__note')
        else:
            return Material.objects.none()
...
class MaterialGroupMaterialsApiDetailView(HeliumAPIView, RetrieveModelMixin, UpdateModelMixin, DestroyModelMixin):
    ...
    def put(self, request, *args, **kwargs):
        ...
        logger.info(f"Material {kwargs['pk']} updated for user {request.user.pk}")

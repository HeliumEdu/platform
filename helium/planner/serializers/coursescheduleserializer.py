...
def get_gated_schedules(schedules, request):
    ...
    understands: at most one **weekly** schedule per course. Cycle (rotating) rows are dropped
    entirely — there's nothing meaningful for such a client to render for a rotating pattern, and
    it wouldn't understand the cycle config fields. Dropping cycle rows first also means the
    single-schedule truncation naturally *prefers a weekly row*: a course with a weekly + a cycle
    schedule hands the client its weekly one, never the cycle row (which, having empty
    `days_of_week`, would render as no meetings at all).
...
class CourseScheduleSerializer(serializers.ModelSerializer):
    """
    A class's schedule. Two shapes, discriminated by `cycle_length`:

    - **Weekly** (`cycle_length` null): defined by `days_of_week` and the per-day
      `*_start_time` / `*_end_time` fields — the writable, canonical representation.
    - **Cycle** (rotating / A-B / block, `cycle_length` set): defined by
      `cycle_length` + `anchor_date` + `cycle_slots`; `days_of_week` and the per-day
    ...
    def validate(self, attrs):
        ...
        if cycle_length is not None:
            # Same gate as multi-schedule: a real client below it can't manage a cycle schedule,
            # so don't let it create one. `request is None` (import) and at/above-gate clients
            # are unaffected.
            if request is not None and not client_version_gte(request, ADVANCED_SCHEDULES_MIN_VERSION):
                raise ValidationError('Cycle schedules require a newer client version.')
            ...
            if attrs.get('anchor_date') is not None or attrs.get('cycle_slots') is not None:
                raise ValidationError("'anchor_date' and 'cycle_slots' are only valid when 'cycle_length' is set.")

...
function HeliumCalendar() {
    ...
    this.drop_calendar_item = function (event) {
        ...
        if (event.calendar_item_type === 0) {
            ...
            self.ajax_calls.push(helium.planner_api.get_homework(callback, event.course.course_group, event.course.id, event.id));
        }
            ...
            if (($("#calendar").fullCalendar("getView").name === "list" || $("#calendar-" + h_e_str + "-checkbox-" + h_e_id).is(":checked") === calendar_item.completed || calendar_item.completed === undefined)) {
                ...
                if (!self.edit) {
                    ...
                    var callback = function (data) {
                        if (helium.data_has_err_msg(data)) {
                            ...
                            if (calendar_item_fields.calendar_item_type === 1) {
                                ...
                                $("#homework-class").val(calendar_item_fields.course.id);
                    ...
                    if (calendar_item.calendar_item_type === 0) {
                        ...
                        helium.planner_api.get_homework(callback, calendar_item.course.course_group, calendar_item.course.id, calendar_item.id, true, false);
                    } else {
                        self.loading_div.spin(false);
                        self.init_calendar_item = false;
                        self.edit = false;
    ...
    this.refresh_filters = function () {
        ...
        if (calendar_ids === "" && course_ids === "" && category_names === "" && !$("#calendar-filter-complete").children().find("input").prop("checked") && !$("#calendar-filter-incomplete").children().find("input").prop("checked") && !$("#calendar-filter-overdue").children().find("input").prop("checked") && !$("#calendar-filter-homework").children().find("input").prop("checked") && !$("#calendar-filter-events").children().find("input").prop("checked") && !$("#calendar-filter-external").children().find("input").prop("checked")) {
    ...
    this.refresh_calendar_items = function () {
        ...
        // filter_show_homework
        // filter_show_events
        // filter_show_external
        // filter_courses
        // filter_categories
        // filter_complete
        // filter_overdue
        ...
        $("#calendar").fullCalendar({
            ...
            eventRender: function (event, element) {
                ...
                if (event.url === undefined) {
                    ...
                    course_string = event.calendar_item_type === 1 ? ((event.course.website.replace(/\s/g, "").length > 0 ? "<a target=\"_blank\" href=\"" + event.course.website + "\">" : "") + event.course.title + (event.course.website.replace(/\s/g, "").length > 0 ? "</a>" : "")) : "";
                    element.qtip({
                        content: {
                            title: "<strong>" + event.title_no_format + "</strong> on " + start,
                            text: "<div class=\"row\"><div class=\"col-xs-12\"><strong>When:</strong> " + start + (event.show_end_time && end ? (" to " + end) : "") + "</div></div>" + (event.calendar_item_type === 1 ? "<div class=\"row\"><div class=\"col-xs-12\"><strong>Class Info:</strong> " + (event.category !== null && event.category.title !== "Uncategorized" ? (event.category.title + " for ") : "") + course_string + (event.course.room.replace(/\s/g, "").length > 0 ? " in " + event.course.room : "") + "</div></div>" : "") + (event.materials.length > 0 && helium.calendar.get_titles_from_data(event.materials) ? "<div class=\"row\"><div class=\"col-xs-12\"><strong>Materials:</strong> " + helium.calendar.get_titles_from_data(event.materials) + "</div></div>" : "") + (event.calendar_item_type === 1 && event.completed && event.current_grade !== "-1/100" ? "<div class=\"row\"><div class=\"col-xs-12\"><strong>Grade:</strong> " + helium.grade_for_display(event.current_grade) + "</div></div>" : "") + (event.comments.replace(/\s/g, "").length > 0 ? "<div class=\"row\"><div class=\"col-xs-12\"><strong>Comments:</strong> " + helium.get_comments_with_link(event.comments) + "</div></div>" : "") + (event.attachments.length > 0 ? "<div class=\"row\"><div class=\"col-xs-12\"><strong>Attachments:</strong> " + helium.calendar.get_titles_from_data(event.attachments) + "</div></div>" : "")
                        },
                        hide: {
        ...
        self.ajax_calls.push(helium.planner_api.get_courses(function (data) {
            if (helium.data_has_err_msg(data)) {
                ...
                $("#filter-clear").on("click", function () {
                    $("#filter-button-title").html("Filter");
                    $.each($("[id^='calendar-filter-course-'], [id^='calendar-filter-category-'], #calendar-filter-homework, #calendar-filter-events, #calendar-filter-external, #calendar-filter-complete, #calendar-filter-incomplete, #calendar-filter-overdue"), function () {
    ...
    this.initialize_filters = function (courses) {
        ...
        $("#calendar-filter-list").append("<li class=\"divider\"></li><li id=\"calendar-filter-homework\"><a class=\"checkbox cursor-hover\"><input type=\"checkbox\"/> &nbsp;<span>Assignments</span></a></li><li id=\"calendar-filter-events\"><a class=\"checkbox cursor-hover\"><input type=\"checkbox\"/> &nbsp;<span>Events</span></a></li><li id=\"calendar-filter-external\"><a class=\"checkbox cursor-hover\"><input type=\"checkbox\"/> &nbsp;<span>External</span></a></li><li class=\"divider\"></li><li id=\"calendar-filter-complete\"><a class=\"checkbox cursor-hover\"><input type=\"checkbox\"/> &nbsp;<span>Complete</span></a></li><li id=\"calendar-filter-incomplete\"><a class=\"checkbox cursor-hover\"><input type=\"checkbox\" /> &nbsp;<span>Incomplete</span></a></li><li id=\"calendar-filter-overdue\"><a class=\"checkbox cursor-hover\"><input type=\"checkbox\" /> &nbsp;<span>Overdue</span></a></li>");
    ...
    this.delete_calendar_item = function (event) {
        ...
        if (event.calendar_item_type === 0) {
            ...
            helium.planner_api.delete_homework(callback, event.course.course_group, event.course.id, event.id);
        }
    };
    ...
    this.clone_calendar_item = function (event) {
        ...
        if (event.calendar_item_type === 0) {
            ...
            helium.planner_api.add_homework(function (data) {
                helium.planner_api.get_homework(callback, event.course.course_group, event.course.id, data.id);
            }, event.course.course_group, event.course.id, cloned);
        }
    };
            ...
            if (self.edit) {
                ...
                $.when.apply(this, helium.calendar.ajax_calls).done(function () {
                    ...
                    if (self.current_calendar_item.calendar_item_type === 0) {
                        helium.planner_api.edit_event(function (data) {
                            helium.planner_api.get_event(callback, self.current_calendar_item.id);

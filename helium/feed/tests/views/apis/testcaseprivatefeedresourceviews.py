...
class TestCasePrivateFeedResourceViews(TestCase):
    ...
    def test_disable_private_url(self):
        ...
        self.assertEqual(
            self.client.get(reverse('feed_events_ical', kwargs={'slug': private_slug})).status_code,
            status.HTTP_404_NOT_FOUND)
        self.assertEqual(
            self.client.get(reverse('feed_homework_ical', kwargs={'slug': private_slug})).status_code,
            status.HTTP_404_NOT_FOUND)

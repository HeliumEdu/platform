...
def increment(metric, request=None, response=None, value=1, extra_tags=None):
    ...
    if request:
        tags.extend(
            [f"method:{request.method.upper()}", f"authenticated:{str(request.user.is_authenticated).lower()}"])
...
def request_stop(metrics, response):
    ...
    statsd.timing(f"platform.{metrics['Request-Metric-ID']}.time", metrics['Request-Metric-Millis'], tags=DATADOG_BASE_TAGS)
...
def task_stop(metrics):
    ...
    statsd.timing(f"platform.{metrics['Task-Metric-ID']}.time", metrics['Request-Metric-Millis'], tags=DATADOG_BASE_TAGS)

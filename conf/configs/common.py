...
SPECTACULAR_SETTINGS = {
    ...
    'DESCRIPTION': (
        ...
        "For any non-trivial import, prefer the bulk-import path over N individual POSTs. "
        ...
        "When editing an existing term (or when a single-call bulk import is impractical), POST in "
        "this order so each create succeeds:\n\n"
        ...
        "When the syllabus doesn't state credit hours, use `\"0.00\"` rather than guessing. "
        ...
        "If the syllabus uses total points rather than weighted percentages, set `weight=0` on every "
        "category and let `current_grade` fractions carry the points; `Course.has_weighted_grading` "
        ...
        "- **Off-day times default to `12:00:00`.** For each day a class does NOT meet, send "
        "`00:00:00` for both `_start_time` and `_end_time` so the off-day is unambiguous. Omitted "
